...
Mode 3 (`launcherResultAlwaysVisible`) keeps the item visible in the launcher results even when the typed prompt does not match. This is useful for items that want to react to the current prompt text via the `BTTLauncherLastPrompt` variable.
...
- `BTTLauncherLastPrompt` — the full current prompt text.
- `BTTLauncherLastPromptKeyword` — the first whitespace-separated word of the prompt (useful when combined with display mode 4).
- `BTTLauncherLastPromptAfterKeyword` — the remainder of the prompt after the first word.
