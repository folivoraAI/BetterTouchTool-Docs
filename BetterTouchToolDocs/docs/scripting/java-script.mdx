---
title: "Using JavaScript (not JXA)"
sidebar_position: 3
slug: "/docs/scripting/java-script"
---
## Using Real JavaScript in BTT

**NOTE**: If you are using the floating webview action in BTT have a look here instead: [Floating Webview JavaScript](/docs/webview/javascript)
...
### Using npm packages / external modules: `require()`
Since BTT 6.735 the engine offers a CommonJS-style `require()`. `require("bettertouchtool")` loads the
bundled copy of the [official npm package](/docs/scripting/node-js-library) (typed helpers, action & trigger
builders). Other modules must be bundled into a single CommonJS file (e.g. `esbuild --format=cjs
--platform=browser`) and can then be loaded by name from
`~/Library/Application Support/BetterTouchTool/JavaScriptModules/<name>.js` or by file path -
see [Node.js / TypeScript Library](/docs/scripting/node-js-library) for the recipe. ES `import` statements
are not supported.

## 8.) Using the JavaScript to call BTT scripting functions
All BetterTouchTool scripting functions can be accessed from JavaScript.
...
async function someFunctionName() {
  ...
  // if you don't care about the result, setting wait_for_reply to false can make execution a bit faster (default is true)
  return result;
}
```
...
```javascript
async function someFunctionName() {
  let result = await delete_trigger({uuid: '2F34005D-4537-464D-94E9-A7F42DA39DF1'});
  return result;
}
```
---
...
async function someFunctionName() {
  let result = await export_preset({
    name: 'the_preset_name',
    compress: 1,
    includeSettings: 0,
    outputPath: '~/Downloads',
    comment: 'this is a great preset',
    link: 'https://someoptionallink',
    minimumVersion: '4.123'
  });
  return result;
}
```
...
This will show a BTT trigger identified by its UUID in the BetterTouchTool user interface.
...
```javascript
async function someFunctionName() {
  // this example sets plain "hello" for apps that only support plain text
  // and a HTML formatted "hello world" for apps that support pasting HTML formatted content
  let result = await set_clipboard_contents({
    contents: ["hello", "<b>hello</b> world"],
...
* **selected**: Optional: if set to true, this will return the currently selected items from the open clipboard manager window, in the order they were selected. When using this parameter, **start**, **numberOfItems** and **snippetGroup** are ignored. Returns an empty items array if no clipboard manager window is open or no items are selected.

```javascript
async function retrieveItems() {
  // additional supported parameters: format, snippetGroup, asBase64 and selected
  // if format is set to "all", it will return all stored information about the requested items,
  // apart from "all" it supports any pasteboard UTI
  return get_items_from_clipboard_manager({start: 0, numberOfItems: 5});
}

async function retrieveSelectedItems() {
  // retrieve the currently selected items from the open clipboard manager, in selection order
  return get_items_from_clipboard_manager({selected: true});
}
```
...
async function pasteSomeItems() {
  ...
  return paste_clipboard_manager_items_with_uuids({
    uuids: "0BD5D9D6-B76D-4E91-99E9-5B59F9067C44,A9880066-4438-4CD2-A103-FB7E3CAD47FC"
  });
}
```
...
### **show_simple_json_format_menu**
Shows a menu defined via [Simple JSON Format](/docs/scripting/simple-format)
...
```javascript
async function someFunctionName() {
  // this returns something that you have saved previously (e.g. via another temporary script or by directly editing keychain)
