...
BetterTouchTool 4.764 introduced a new "Simple JSON Format", which can currently be used with the predefined actions "Show Custom Context Menu (New)" and "Choose From List" and with Floating Menus. It allows to retrieve the content for those menus dynamically using Java Script & JSON.
It allows you to use any of [BTT's Java Script capabilities](1106_java_script.md) to produce the output JSON.

- [BetterTouchTool Simple JSON Format](#bettertouchtool-simple-json-format)
  - [How to create dynamic menus based on the Simple JSON Format](#how-to-create-dynamic-menus-based-on-the-simple-json-format)
    - [Alternative: Full JSON Definition](#alternative-full-json-definition)
  - [Supported Container Properties](#supported-container-properties)
  - [Supported Item Properties](#supported-item-properties)
...
- [Allowed Properties:](#allowed-properties)
  ...
  - [templateitemuuid](#templateitemuuid)
  - [icon](#icon)
  - [action](#action)
    - [named](#named)
    - [uuid](#uuid)
...
- [Examples](#examples)
  - [Example: Create an AI / App Launcher:](#example-create-an-ai--app-launcher)
  - [Example: Get result from Apple Shortcut:](#example-get-result-from-apple-shortcut)
  - [Example: Execute multiple actions from one item (example types abc)](#example-execute-multiple-actions-from-one-item-example-types-abc)
  - [Example: Fetch menu from some server:](#example-fetch-menu-from-some-server)
  - [Example: Usage With Custom Menu Bar Items](#example-usage-with-custom-menu-bar-items)
  - [Example: Floating Menu App Switcher](#example-floating-menu-app-switcher)
...
Then you can have a async function that returns a JSON. You can use any of BTT's [scripting functions](1106_java_script.md) inside that Java Script (e.g. run Shell or Apple Scripts and much more). **Note** Make sure to set the async function to call in the separate text field.
...
## setvariable
this allows to set a variable value before the action is executed. If the simple string based action format is used, the **setvariable** must be at the top level of the item definition. If you have multiple actions all string based actions will use that setvariable definition.
...
## templateitemuuid
This is only for use within [Floating Menus](1600_floating_menus.md), it allows you to use the configuration of any Floating Menu Item you have created in the BTT UI as a template. 
...
## Action Categories
Some items in BTT support different action categories, e.g. a floating menu button that can trigger one action on left-click but another action on right-click. If you want to make us of that, instead of defining your action via the "action" property, you need to define it via the "actions" property.
...
Possible action trigger category names are listed here, however they availability depends on the BTT feature you are using:
* standard
* rightclick
* hover
* longpress
